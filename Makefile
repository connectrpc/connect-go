...
GO_BINS := $(GO_BINS)  cmd/protoc-gen-go-connect
GO_ALL_REPO_PKGS := ./...
GIT_FILE_IGNORES := $(GIT_FILE_IGNORES) cover.out *.pprof *.svg
...
bufgeneratedeps:: $(BUF) $(PROTOC_GEN_GO) installprotoc-gen-go-connect
