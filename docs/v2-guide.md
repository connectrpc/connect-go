...
* **Compressors:** Listed in order of preference (most preferred first). Clients and handlers advertise this order in `Accept-Encoding`. A handler responds using the first encoding the client listed that it also supports. Use `WithSendCompression` to set the outgoing client compression.
