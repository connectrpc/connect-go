...
func serverConstructor(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	for _, method := range service.Methods {
		...
		if method.Desc.IsStreamingServer() || method.Desc.IsStreamingClient() {
			...
			g.P("if err != nil {")
			g.P("if _, ok := ", connectPackage.Ident("AsError"), "(err); !ok {")
			g.P("if ", errorsIs, "(err, ", contextCanceled, ") {")
			g.P("err = ", connectPackage.Ident("Wrap"), "(", connectPackage.Ident("CodeCanceled"), ", err)")
			g.P("}")
			g.P("if ", errorsIs, "(err, ", contextDeadlineExceeded, ") {")
			g.P("err = ", connectPackage.Ident("Wrap"), "(", connectPackage.Ident("CodeDeadlineExceeded"), ", err)")
			g.P("}")
			g.P("}")
			g.P("}")
			g.P("_ = sender.Close(err)")
